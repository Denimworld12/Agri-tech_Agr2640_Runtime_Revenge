...
export function Avatar(props) {
  ...
  useEffect(() => {
    ...
    // If no audio (like the error fallback message), show animation for 3 seconds then end
    if (!message.audio) {
    ...
    audioRef.current = audio; // Update ref for immediate useFrame access
    setAudio(audio); // Update state for React tracking
  ...
  // Fix: Filter out tracks for missing bones
  useEffect(() => {
    animations.forEach((clip) => {
      const tracksToRemove = [];
      clip.tracks.forEach((track) => {
        if (
          track.name.includes("RightToe_End_end") ||
          track.name.includes("LeftToe_End_end") ||
